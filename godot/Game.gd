...
func _ready() -> void:
	...
	combo_progress_bar.max_value = 2 * conductor.get_crotchet()
...
func _process(delta: float) -> void:
	...
	if not editor:
		...
		if Input.is_action_just_pressed("editlevel") and not $Editor.editor_mode in [1, 4]:
			...
			conductor.sound_object.play(editor_hud.editor_beat * conductor.get_crotchet())
			editor_hud.editor_notes.append((conductor.notes[0].duplicate()))
			editor_hud.editor_notes.append((conductor.notes[1].duplicate()))
			editor_hud.editor_notes.append((conductor.notes[2].duplicate()))
			conductor.time = null
...
func process_game(delta: float) -> void:
	...
	if combo == 0:
		combo_timer_goal = ceil(conductor.song_position().get_time_in_beats()) * conductor.get_crotchet() + 2.0 * conductor.get_crotchet()
	else:
		process_combo_timer()
		combo_progress_bar.value = max(combo_timer_goal - conductor.song_position().get_time_in_seconds(), 0)
		combo_progress_bar.value = 2 * conductor.get_crotchet()
...
func _on_beat_happened(beat_number):
	...
	tween_background.tween_property(background_flash, "self_modulate:a", max(combo / 50, 0.3), conductor.get_crotchet() * 0.1)
	tween_background.tween_property(background_flash, "self_modulate:a", 0.2, conductor.get_crotchet() * 0.75)
	if combo > 0: combo_timer_go_down = true
	if combo_amount_text.rotation < 0.0:
		var tween = create_tween()
		tween.set_parallel(true)
		tween.tween_property(combo_amount_text, "rotation", 10 * max(combo, 2) / 50.0, conductor.get_crotchet()).set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_ELASTIC)
		tween.tween_property(hit_notification, "rotation", -5 * max(combo, 2) / 50.0, conductor.get_crotchet()).set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_ELASTIC)
	else:
		var tween = create_tween()
		tween.set_parallel(true)
		tween.tween_property(combo_amount_text, "rotation", -10 * max(combo, 2) / 50.0, conductor.get_crotchet()).set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_ELASTIC)
...
func _on_Timer_timeout() -> void:
	conductor.connect("beat_plus_offset_happened", Callable(self, "_on_beat_plus_offset_happened"))
	conductor.connect("beat_happened", Callable(self, "_on_beat_happened"))
	music_player.play(conductor.get_crotchet() * 0)
	song_started = true
...
func hit_note(imprecision, lateness, time, combo_leniency:=1.0):
	...
	if previous_combo != 0 and combo == 0:
		...
		live_luna_reaction.speed_scale = 1.0 / conductor.get_crotchet()
	elif combo <= 100:
...
func read_level():
	...
	if error == OK:
		...
		for object in data.notes:
			if object.type == NoteData.get_type():
...
func setup_note_nodes(note_array: Array):
	...
	for note_data in note_array:
		...
		new_note_node.seconds_from_previous = previous_beat_time * conductor.get_crotchet()
		...
		if previous_note_node != null:
			previous_note_node.seconds_for_next = new_note_node.hit_beat_number * conductor.get_crotchet()
