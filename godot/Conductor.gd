...
func song_position()->BpmTime:
	...
	if time == null:
		time = BpmTime.new_from_seconds(bpm, calculated_current_time)
	if sound_object.playing:
		time = BpmTime.new_from_seconds(bpm, max(calculated_current_time, time.get_time_in_seconds()))
	return time
