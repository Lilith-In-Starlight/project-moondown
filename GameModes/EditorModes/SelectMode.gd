...
func draw_selections(editor:EditorGameMode):
	...
	if editor.editor_mode == 2:
		match selection_cursor:
			0:select_mode_cursor.rect_position.x = 378
			1:select_mode_cursor.rect_position.x = 480
			2:select_mode_cursor.rect_position.x = 578
	...
	for idx in editor_selections.size() + plus:
		...
		if idx != editor_selections.size():
			child.rect_position.x = lerp(child.rect_position.x, selection.start.x * selection_display.rect_size.x / 3.0, 0.2)
			child.rect_position.y = lerp(child.rect_position.y, selection.end.y * - Global.SCROLL_SPEED + 513 - 32 + editor.editor_beat * Global.SCROLL_SPEED, 0.2)
			child.rect_size.x = (selection.end.x - selection.start.x + 1) * selection_display.rect_size.x / 3.0
			child.rect_size.y = (selection.end.y - selection.start.y) * Global.SCROLL_SPEED + 64
		else :
			var starty: = min(editor.editor_beat, selection.start.y)
			var startx: = min(selection_cursor, selection.start.x)
			var endy: = max(editor.editor_beat, selection.start.y)
			var endx: = max(selection_cursor, selection.start.x)
			child.rect_position.x = lerp(child.rect_position.x, startx * selection_display.rect_size.x / 3.0, 0.2)
			child.rect_position.y = lerp(child.rect_position.y, 513 - 32 + (endy - editor.editor_beat) * - Global.SCROLL_SPEED, 0.2)
			child.rect_size.x = lerp(child.rect_size.x, (endx - startx + 1) * selection_display.rect_size.x / 3.0, 0.2)
			child.rect_size.y = lerp(child.rect_size.y, (endy - starty) * Global.SCROLL_SPEED + 64, 0.2)
